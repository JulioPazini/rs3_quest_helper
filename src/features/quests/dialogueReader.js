...
function findDialogueBox(buf, width, height) {
  ...
  for (let y = 10; y < height - HEADER_HEIGHT - 30; y += STEP) {
    for (let x = 10; x < width - MIN_DIALOGUE_WIDTH; x += STEP) {
      ...
      // Estimate box height: HEADER + options + small bottom padding
 ...
 * @param {string} [optionText]  Optional text shown alongside arrow
 */
function drawHighlight(box, optionNumber, optionText) {
  if (!isAlt1Available()) return;
  try {
...
function poll() {
  ...
  // Determine the first option to highlight (skip 'any' markers)
  const target = _requiredOptions.find((r) => /^\d+$/.test(r));
  if (!target) {
    // All required options are 'any' (✓/~) — no specific highlight needed
    clearOverlay();
    return;
  }
  ...
  // Find the option text for the arrow label
  const optEntry = _dialogueOptions.find((o) => String(o.option).trim() === String(optionNumber));
  drawHighlight(box, optionNumber, optEntry?.text ?? '');
}
